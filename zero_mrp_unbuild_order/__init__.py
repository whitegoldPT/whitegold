...
def _configure_journals(env):
    ...
    for company in env['res.company'].search([('chart_template', '!=', False)]):
        ...
        if 'property_stock_account_unbuild_profit_loss_id' in template_data:
            data = {'property_stock_account_unbuild_profit_loss_id': template_data['property_stock_account_unbuild_profit_loss_id']}

...
class MrpProduction(models.Model):
    ...
    def create(self, vals):
        ...
        for material in result.bom_id.direct_material_ids:
               material_vals = {
                    'routing_workcenter_id': material.routing_workcenter_id.id, 
        ...
        for labour in result.bom_id.labour_cost_ids:
                labour_vals = {
                    'routing_workcenter_id': labour.routing_workcenter_id.id, 
        ...
        for overhead in result.bom_id.overhead_cost_ids:
                overhead_vals = {
                    'routing_workcenter_id': overhead.routing_workcenter_id.id, 
        ...
        for rec in self:
            for material in rec.direct_material_ids:
                for order in result:
                    ...
                    if material.routing_workcenter_id.workcenter_id == order.workcenter_id and \
                        rec.bom_id.custom_workorder_selection == 'workcenter':
                        workorder_materil = workder_job_costline_obj.create(material_vals)
                        material.work_order_line_id = workorder_materil
                    elif material.routing_workcenter_id == order.operation_id and \
                        rec.bom_id.custom_workorder_selection == 'operation':
                        workorder_materil = workder_job_costline_obj.create(material_vals)
                        material.work_order_line_id = workorder_materil
            for labour in rec.labour_cost_ids:
                for order in result:
                    ...
                    if labour.routing_workcenter_id.workcenter_id == order.workcenter_id and \
                        rec.bom_id.custom_workorder_selection == 'workcenter':
                        workorder_labour = workder_job_costline_obj.create(labour_vals)
                        labour.work_order_line_id = workorder_labour
                    elif labour.routing_workcenter_id == order.operation_id and \
                        rec.bom_id.custom_workorder_selection == 'operation':
                        workorder_labour = workder_job_costline_obj.create(labour_vals)
                        labour.work_order_line_id = workorder_labour
            for overhead in rec.overhead_cost_ids:
                for order in result:
                    ...
                    if overhead.routing_workcenter_id.workcenter_id == order.workcenter_id and \
                        rec.bom_id.custom_workorder_selection == 'workcenter':
                        workorder_overhead = workder_job_costline_obj.create(overhead_vals)
                        overhead.work_order_line_id = workorder_overhead
                    elif overhead.routing_workcenter_id == order.operation_id and \
                        rec.bom_id.custom_workorder_selection == 'operation':
                        workorder_overhead = workder_job_costline_obj.create(overhead_vals)
                        overhead.work_order_line_id = workorder_overhead
        return result
    ...
    def _plan_workorders(self, replan=False):
        result = super(MrpProduction, self)._plan_workorders(replan)
        workder_job_costline_obj = self.env['workorder.job.cost.line']
        for rec in self:
            for material in rec.direct_material_ids:
                for order in rec.workorder_ids:
                    material_vals = {
                            'routing_workcenter_id': material.routing_workcenter_id.id, 
                            'product_id': material.product_id.id, 
                            'product_qty': material.product_qty,
